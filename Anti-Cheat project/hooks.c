...
BOOL CheckEATHooksForModule(HANDLE hProcess, const char* moduleName) {
    ...
    DWORD moduleSize = 0;
    if (!GetRemoteModuleBounds(hProcess, moduleName, &moduleBase, &moduleSize)) {
        return FALSE;
    }
    if (!moduleBase) { printf("[-] Module base in remote process is 0."); exit(1); }

    // Read DOS header
    IMAGE_DOS_HEADER dosHeader;
    if (!ReadProcessMemory(hProcess, (PVOID)moduleBase, &dosHeader, sizeof(dosHeader), NULL)) {
        return FALSE;
    }

    // Read NT headers
    IMAGE_NT_HEADERS ntHeaders;
    if (!ReadProcessMemory(hProcess, (PBYTE)moduleBase + dosHeader.e_lfanew, &ntHeaders, sizeof(ntHeaders), NULL)) {
        return FALSE;
    }

    DWORD exportDirRVA = ntHeaders.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress;
    if (exportDirRVA == 0) {
        // Module has no exports (skip silently)
        return FALSE;
    }
    ...
    if (!ReadProcessMemory(hProcess, (PBYTE)moduleBase + exportDirRVA, &exportDir, sizeof(exportDir), NULL)) {
        return FALSE;
    }

    // Read functions array
    PDWORD functionArray = (PDWORD)malloc(exportDir.NumberOfFunctions * sizeof(DWORD));
    if (!ReadProcessMemory(hProcess, (PBYTE)moduleBase + exportDir.AddressOfFunctions,
        functionArray, exportDir.NumberOfFunctions * sizeof(DWORD), NULL)) {
        free(functionArray);
        return FALSE;
    }

    // Read name ordinals
    PWORD nameOrdinalsArray = (PWORD)malloc(exportDir.NumberOfNames * sizeof(WORD));
    ReadProcessMemory(hProcess, (PBYTE)moduleBase + exportDir.AddressOfNameOrdinals,
        nameOrdinalsArray, exportDir.NumberOfNames * sizeof(WORD), NULL);

    // Read names RVAs
    PDWORD namesArray = (PDWORD)malloc(exportDir.NumberOfNames * sizeof(DWORD));
    ReadProcessMemory(hProcess, (PBYTE)moduleBase + exportDir.AddressOfNames,
        namesArray, exportDir.NumberOfNames * sizeof(DWORD), NULL);

    // Build name lookup table
    char** functionNames = (char**)calloc(exportDir.NumberOfFunctions, sizeof(char*));
    for (DWORD i = 0; i < exportDir.NumberOfNames; i++) {
        WORD ordinal = nameOrdinalsArray[i];
        if (ordinal < exportDir.NumberOfFunctions) {
            functionNames[ordinal] = (char*)malloc(256);
            ReadProcessMemory(hProcess, (PBYTE)moduleBase + namesArray[i],
                functionNames[ordinal], 256, NULL);
        }
    }
    ...
    for (DWORD i = 0; i < exportDir.NumberOfFunctions; i++) {
        DWORD functionRVA = functionArray[i];
        if (functionRVA == 0) continue;

        if (functionRVA >= sizeOfImage) {
            const char* name = functionNames[i] ? functionNames[i] : "Ordinal_only";
            printf("[!] EAT Hook in %s: %s (index %lu, RVA 0x%X, size 0x%X)\n",
                moduleName, name, i, functionRVA, sizeOfImage);
            hookFound = TRUE;
        }
    }
...
BOOL CheckIATHooks(DWORD targetPID) {
    ...
    if (hookCount == 0) {
        printf("[+] Scan complete. No IAT hooks found.\n");
        return FALSE;
    }
    else {
